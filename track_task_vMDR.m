function [recorded_grip, recorded_time, nb_trials, el_block_time, el_trial_time, val_save_trial, el_reach_time, ...
...
outcome_block = strings(1,length(nf_trials));
hit_times = zeros(length(nf_trials), 6);
test = 0;
test2=0;
...
recorded_time = zeros(1,1000000);
val_save = 1;

if I == t_param.random_block
    used_bar_real = rand_bar_real;
else
...
motor_start = clock;
 ...
 while (1)
    ...
    recorded_time(1, val_save) = toc(start_time);
...
%    disp(val_save)
   ...
   % When the ball leave the home zone, start timer for each trial
    ...
    while ( Y_c > t_param.base_bar_y_pos && test == 0 && (Y_c < all_bars{barreal}.Vertices(3,2) || Y_c > all_bars{barreal}.Vertices(1,2)))
        test2 = 1;
        delete(ball)
        Y_c = map_bar(t_param.joy, relax, Max_force);
        recorded_grip(1, val_save) = Y_c;
...
%        disp(val_save)
        ...
        if nb_trials == 0 
            if val_save > 30 && Y_c > all_bars{1}.Vertices(3,2)
               [~,idx]=min(abs(recorded_time(val_save-30:val_save-1)-(recorded_time(val_save-1)-max_time))); % Permet de trouver l'indexe de la valeur val_save
                                                                                                            %qui a été enregistrée 200ms avant celle en cours                                                                                                   
                % if val_save > max_time && std(recorded_grip((val_save - max_time):(val_save-1))) < 0.000001
                if std(recorded_grip((val_save-1-(30-idx)):(val_save-1))) < 0.001
